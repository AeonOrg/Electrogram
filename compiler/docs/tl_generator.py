...
class TLDocGenerator:
    def __init__(self, tl_files: list[Path]):
        ...
        for c in self.combinators:
            ns, name = (
                c.qualname.split(".") if "." in c.qualname else ("", c.qualname)
            )
            ...
            with tl_file.open("r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if combinator_match := COMBINATOR_RE.match(line):
                        ...
                        combinators.append(
                            Combinator(section, qualname, cid, args, qualtype, line)
                        )
        return combinators
    ...
    def generate_tree(self, name: str) -> str:
        res = [f"| **{name.rsplit('.', maxsplit=1)[-1]}**"]
        ...
        res.extend(
            self._generate_tree_recursive(name, 0, "", set(), is_base=is_base)
        )
        return "\n".join(res)
    ...
    def _generate_tree_recursive(
        ...
        # Prevent cycles
        if name in visited and depth > 5:
            return []
        ...
        if is_vector_item:
            ...
            if name in self.base_to_constructors:
                ...
                for i, constr in enumerate(constrs):
                    ...
                    res.append(
                        f"| {prefix}{'└── ' if is_last else '├── '}{self.get_type_link(constr)}"
                    )
                    if depth < 4:
                        res.extend(
                ...
                res.extend(
                    self._generate_tree_recursive(
                        name, depth + 1, prefix + "    ", new_visited, is_base=False
                    )
                )
            return res
        ...
        filtered_args = [
            a for a in c.args if not (a.name.startswith("flags") and a.type == "#")
        ]
    ...
    def get_default_value(
        ...
        if arg_type.startswith("Vector<"):
            ...
            val = self.get_default_value(
                arg_name, inner, depth + 1, visited.copy(), minimal=minimal
            )
            indent = "    " * (depth + 1)
            item_indent = "    " * (depth + 2)
            if inner not in CORE_TYPES:
        ...
        if arg_type == "InputPeer":
            return "await app.resolve_peer(chat_id)"
        if arg_type == "InputUser":
            return "await app.resolve_user(chat_id)"
        if arg_type == "InputChannel":
        ...
        if c:
            args_code = []
            for arg in c.args:
                if arg.name.startswith("flags") and arg.type == "#":
                    continue
                val = self.get_default_value(
                    arg.name, arg.type, depth + 1, visited.copy(), minimal=minimal
                )
    ...
    def generate_example(self, name: str, minimal: bool = False) -> str:
        ...
        if not c:
            return ""
        if c.section not in {"functions", "types"}:
            return ""
        name_only = name.rsplit(".", maxsplit=1)[-1]
